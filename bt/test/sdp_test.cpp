...
int main() {
    ...
    {   // 9. ROLE: Sdp::setRole(SINK) publishes the AudioSink record (0x110B) instead of AudioSource, beside the AVRCP
        ...
        auto r = serve({ 0x06, 0x00, 0x21, 0x00, 0x10, 0x35, 0x03, 0x19, 0x11, 0x0B, 0x03, 0xF0, 0x35, 0x06, 0x09, 0x00, 0x04, 0x09, 0x00, 0x09, 0x00 }, 1008);
        ...
        CHECK(pdl && profile);
        auto none = serve({ 0x06, 0x00, 0x22, 0x00, 0x10, 0x35, 0x03, 0x19, 0x11, 0x0A, 0x03, 0xF0, 0x35, 0x06, 0x09, 0x00, 0x04, 0x09, 0x00, 0x09, 0x00 }, 1008);
        CHECK(none.size() >= 7 && none[0] == 0x07 && none[5] == 0x00 && none[6] == 0x02);   // AttributeListsByteCount 2: an empty DES
        auto avrcp = serve({ 0x06, 0x00, 0x23, 0x00, 0x10, 0x35, 0x03, 0x19, 0x11, 0x0C, 0x03, 0xF0, 0x35, 0x06, 0x09, 0x00, 0x04, 0x09, 0x00, 0x09, 0x00 }, 1008);
        CHECK(avrcp.size() > 20);                                                                   // the AVRCP Target record is still served
        Sdp::setRole(Sdp::SOURCE);
        auto src = serve({ 0x06, 0x00, 0x24, 0x00, 0x10, 0x35, 0x03, 0x19, 0x11, 0x0A, 0x03, 0xF0, 0x35, 0x06, 0x09, 0x00, 0x04, 0x09, 0x00, 0x09, 0x00 }, 1008);
        CHECK(src.size() > 20);
        auto sinkOff = serve({ 0x06, 0x00, 0x25, 0x00, 0x10, 0x35, 0x03, 0x19, 0x11, 0x0B, 0x03, 0xF0, 0x35, 0x06, 0x09, 0x00, 0x04, 0x09, 0x00, 0x09, 0x00 }, 1008);
        CHECK(sinkOff.size() >= 7 && sinkOff[5] == 0x00 && sinkOff[6] == 0x02);                  // and the sink record is NOT served as a source
    }
    {   // 10. ROLE + AVRCP CATEGORY: the AVRCP Target record's 0x0311 SupportedFeatures depends on the role.  Measured on
        //     the bench 2026-09-09: an iPhone streams to our sink but never sends SetAbsoluteVolume -- AVRCP 1.4 SS6.13
        //     makes absolute volume a Category 2 (monitor/amplifier, bit 1) feature and iOS gates on the TARGET's
        //     SupportedFeatures.  So as a SINK the target advertises 0x0002; as a SOURCE it stays 0x0001 (Category 1,
        //     player/recorder) -- byte-identical to test 2, which is what every existing consumer and gate reads.
        Sdp::setRole(Sdp::SINK);
        auto tgt = serve({ 0x06, 0x00, 0x01, 0x00, 0x10, 0x35, 0x03, 0x19, 0x11, 0x0C, 0x00, 0x26, 0x35, 0x06, 0x09, 0x00, 0x09, 0x09, 0x03, 0x11, 0x00 });
        CHECK(eq(tgt, { 0x07, 0x00, 0x01, 0x00, 0x1A, 0x00, 0x17, 0x35, 0x15, 0x35, 0x13,
                        0x09, 0x00, 0x09, 0x35, 0x08, 0x35, 0x06, 0x19, 0x11, 0x0E, 0x09, 0x01, 0x04,
                        0x09, 0x03, 0x11, 0x09, 0x00, 0x02, 0x00 }));
        // the AudioSink record's OWN 0x0311 is untouched: 0x0001, headphone.  Only the AVRCP Target's moves.
        auto snk = serve({ 0x06, 0x00, 0x02, 0x00, 0x0D, 0x35, 0x03, 0x19, 0x11, 0x0B, 0x00, 0x26, 0x35, 0x03, 0x09, 0x03, 0x11, 0x00 });
        CHECK(eq(snk, { 0x07, 0x00, 0x02, 0x00, 0x0D, 0x00, 0x0A, 0x35, 0x08, 0x35, 0x06, 0x09, 0x03, 0x11, 0x09, 0x00, 0x01, 0x00 }));
        Sdp::setRole(Sdp::SOURCE);
        auto src = serve({ 0x06, 0x00, 0x03, 0x00, 0x10, 0x35, 0x03, 0x19, 0x11, 0x0C, 0x00, 0x26, 0x35, 0x06, 0x09, 0x00, 0x09, 0x09, 0x03, 0x11, 0x00 });
        CHECK(eq(src, { 0x07, 0x00, 0x03, 0x00, 0x1A, 0x00, 0x17, 0x35, 0x15, 0x35, 0x13,
                        0x09, 0x00, 0x09, 0x35, 0x08, 0x35, 0x06, 0x19, 0x11, 0x0E, 0x09, 0x01, 0x04,
                        0x09, 0x03, 0x11, 0x09, 0x00, 0x01, 0x00 }));
    }
