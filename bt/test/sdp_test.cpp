...
int main() {
    ...
    {   // 2. The Shokz's AVRCP-Target query (frame 677; measured again on our own bench 2026-09-07 once the AVCTP channel
        // was accepted): pattern {0x110C}, attrs {0x0009, 0x0311}.  NEW-34 piece 3: we now HAVE an AVRCP Target record --
        // 0x0009 = { {AVRemoteControl 0x110E, 1.4} }, 0x0311 = 0x0001 -- and the headset sends no AV/C until it gets one.
        auto r = serve({ 0x06, 0x00, 0x01, 0x00, 0x10, 0x35, 0x03, 0x19, 0x11, 0x0C, 0x00, 0x26, 0x35, 0x06, 0x09, 0x00, 0x09, 0x09, 0x03, 0x11, 0x00 });
        CHECK(eq(r, { 0x07, 0x00, 0x01, 0x00, 0x1A, 0x00, 0x17, 0x35, 0x15, 0x35, 0x13,
                      0x09, 0x00, 0x09, 0x35, 0x08, 0x35, 0x06, 0x19, 0x11, 0x0E, 0x09, 0x01, 0x04,
                      0x09, 0x03, 0x11, 0x09, 0x00, 0x01, 0x00 }));
    }
    {   // 2b. The Shokz's SECOND query, {0x110E} (the AVRCP profile UUID), same attrs -- matches the SAME target record
        // (0x110E is in its profile descriptor list), so the answer is byte-identical apart from the transaction id.
        auto r = serve({ 0x06, 0x00, 0x02, 0x00, 0x10, 0x35, 0x03, 0x19, 0x11, 0x0E, 0x00, 0x26, 0x35, 0x06, 0x09, 0x00, 0x09, 0x09, 0x03, 0x11, 0x00 });
        CHECK(eq(r, { 0x07, 0x00, 0x02, 0x00, 0x1A, 0x00, 0x17, 0x35, 0x15, 0x35, 0x13,
                      0x09, 0x00, 0x09, 0x35, 0x08, 0x35, 0x06, 0x19, 0x11, 0x0E, 0x09, 0x01, 0x04,
                      0x09, 0x03, 0x11, 0x09, 0x00, 0x01, 0x00 }));
    }
    {   // 2c. A ServiceSearchRequest for PublicBrowseRoot {0x1002} lists BOTH handles, in order.
        auto r = serve({ 0x02, 0x00, 0x03, 0x00, 0x08, 0x35, 0x03, 0x19, 0x10, 0x02, 0x00, 0x10, 0x00 });
        CHECK(eq(r, { 0x03, 0x00, 0x03, 0x00, 0x0D, 0x00, 0x02, 0x00, 0x02, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01, 0x00, 0x01, 0x00 }));
    }
