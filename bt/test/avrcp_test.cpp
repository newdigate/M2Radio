...
int main() {
    ...
    {   // 3. Another AVRCP command (GetCapabilities, PDU 0x10, STATUS ctype) -> NOT IMPLEMENTED, operands echoed.
        const uint8_t gc[] = { 0x30, 0x11, 0x0E, 0x01, 0x48, 0x00, 0x00, 0x19, 0x58, 0x10, 0x00, 0x00, 0x01, 0x03 };
        uint8_t out[64]; bool notif = true; uint16_t n = Avrcp::respond(gc, sizeof gc, out, sizeof out, &notif);
        CHECK(!notif);
        CHECK(eq(out, n, { 0x32, 0x11, 0x0E, 0x08, 0x48, 0x00, 0x00, 0x19, 0x58, 0x10, 0x00, 0x00, 0x01, 0x03 }));
    }
